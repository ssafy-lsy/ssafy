...
# dat배열 쓰는이유?
# get_count() 함수 만들면 4중 for문 --> dat배열쓰면 2중 for문
...
### DAT FINAL 2
``` python
# query(question)가 아파트 주민
# 범죄자 == dat배열로
# 2중 for문 2번으로 풀이

criminal = [[5, 7, 9, 55], [30, 10, 6, 8]]
person = [[1, 2, 3, 4], [5, 7, 10, 15]]

dat = [0] * 100 # 무조건 55보다는 커야된다.
...
for y in range(2):
    for x in range(4):
        dat[criminal[y][x]] = 1 # criminal의 값을 인덱스로

for y in range(2):
    for x in range(4):
        if dat[person[y][x]]: crim += 1
        else: per += 1
...
# 블랙리스트로 -> dat배열 (블랙리스트 값이 몇부터 몇까지인지)
dat = [0] * 100001
